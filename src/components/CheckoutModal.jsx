...
import { 
  ...
  Smartphone, ShieldCheck, ArrowRight, User, AlertCircle, RefreshCw, Phone, Key
} from 'lucide-react';
import { 
  setupRecaptcha,
  sendPhoneOtp,
...
export default function CheckoutModal({ 
  ...
  // Phone Auth State inside Checkout
  ...
  const handleVerifyPhoneOtp = async (e) => {
    ...
    try {
      const res = await verifyPhoneOtp(confirmationResult, otpCode.trim(), clientName.trim(), fullPhone);
      if (res.success && res.user) {
        setCurrentUser(res.user);
        setPhoneMessage('Mobile number verified successfully!');
  ...
  const handleSubmitPayment = async (e) => {
    ...
    if (!currentUser) {
      setErrorMessage('Please verify your mobile number above before submitting payment.');
      return;
    }
    ...
    try {
      const res = await submitShagunPaymentOrder({
        ...
        utr: utr.trim(),
        payerName: payerName.trim() || clientName.trim() || currentUser.displayName || currentUser.phoneNumber,
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
      ...
      <div className="relative w-full max-w-xl rounded-3xl bg-[#0E0C1C] border border-champagne-500/30 p-5 sm:p-8 shadow-2xl space-y-6">
        ...
        {/* reCAPTCHA container for checkout */}
        <div id="checkout-recaptcha-container" ref={recaptchaRef} className="flex justify-center my-1"></div>
        ...
        <div className="text-center space-y-1.5 pt-2">
          ...
          <p className="text-xs text-slate-300">
            Pay auspicious <span className="font-bold gold-gradient-text text-sm">₹1001 Shagun Money</span> to customize, live-edit, and publish your wedding invitation.
          </p>
        </div>
          ...
          {!currentUser ? (
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-champagne-400 font-sans"
                  />
                  <div className="flex gap-1.5">
                    <select
                      value={countryCode}
                      onChange={(e) => setCountryCode(e.target.value)}
                      className="px-2 py-2 rounded-xl bg-white/5 border border-white/15 text-white text-xs font-mono focus:outline-none"
                    >
                    ...
                    <input
                      ...
                      className="flex-1 px-3 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-champagne-400 font-mono"
                    />
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl bg-white hover:bg-slate-100 text-slate-900 font-semibold text-xs shadow-md transition-all flex items-center justify-center gap-2 active:scale-95 cursor-pointer disabled:opacity-50"
                >
                  <Smartphone className="w-4 h-4 text-slate-900" />
                  <span>{phoneLoading ? 'Sending SMS OTP...' : 'Send SMS OTP Code to Verify Phone'}</span>
                </button>
              </div>
            ) : (
              <div className="space-y-3 animate-fadeIn">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 rounded-xl bg-black/60 border border-champagne-400/50 text-center font-mono text-sm tracking-widest text-white placeholder-slate-500 focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={handleVerifyPhoneOtp}
                    disabled={phoneLoading}
                    className="px-4 py-2.5 rounded-xl bg-emerald-500 text-slate-950 font-bold text-xs hover:bg-emerald-400 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {phoneLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-3.5 h-3.5" />}
                    <span>Verify OTP</span>
                  </button>
                </div>
                <div className="flex justify-between text-[11px] text-slate-400">
                  <button type="button" onClick={() => setPhoneStep('phone')} className="underline hover:text-white">Change Number</button>
                  <button type="button" onClick={handleSendPhoneOtp} className="text-champagne-400 hover:underline">Resend OTP</button>
                </div>
              </div>
            )
          ) : (
            <div className="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/10">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-xs font-bold text-white truncate">{currentUser.displayName || 'Client User'}</div>
                <div className="text-[11px] text-champagne-300 font-mono truncate">{currentUser.phoneNumber || currentUser.email}</div>
              </div>
            </div>
          )}
        </div>
        ...
        {isVerifiedByAdmin ? (
          <div className="glass-panel p-6 rounded-2xl border border-emerald-500/40 bg-emerald-950/20 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-300">
                Your ₹1001 Shagun payment has been verified by the admin. You have full access to live customization, 3D unboxing, and publishing.
              </p>
            </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-amber-500/40 bg-amber-950/20 text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-400 pt-1">
                The admin is verifying your ₹1001 Shagun payment. This screen will automatically unlock as soon as verified!
              </p>
            </div>
            ...
            <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-champagne-500/30 bg-black/50 flex flex-col sm:flex-row items-center gap-5">
              ...
              <div className="space-y-2 text-center sm:text-left flex-1">
                <div className="text-[11px] uppercase font-mono tracking-wider text-champagne-400 font-semibold">
                  Scan & Pay ₹1001 Shagun
                </div>
                <div className="font-cinzel text-2xl font-bold gold-gradient-text">
                  ₹1001 INR
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Scan this QR code with <strong>PhonePe, Google Pay, Paytm, BHIM</strong>, or any UPI app to transfer ₹1001 Shagun.
                </p>

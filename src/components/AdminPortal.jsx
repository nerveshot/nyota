...
export default function AdminPortal({ onBackToSite, onPreviewInvitation }) {
  ...
  const handleAdminOneClick = async () => {
    ...
    try {
      ...
      console.error(err);
      setLoginError('Error signing in as Admin.');
    }
  };
        ...
        <div className="relative w-full max-w-md glass-panel p-8 rounded-3xl border border-champagne-500/30 shadow-2xl space-y-6 animate-fadeIn">
          ...
          <div className="space-y-3 pt-2">
            <button
              ...
              className="w-full py-3.5 px-4 rounded-2xl bg-gradient-to-r from-champagne-400 via-amber-500 to-champagne-600 text-slate-950 font-bold text-sm shadow-glow-gold hover:opacity-95 active:scale-95 transition-all flex items-center justify-center gap-2.5"
            >
              <Crown className="w-5 h-5 text-slate-950" />
              <span>1-Click Super Admin Login ({ADMIN_EMAIL})</span>
            </button>

            <button
              onClick={handleGoogleSignIn}
              className="w-full py-3 px-4 rounded-2xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs border border-white/20 active:scale-95 transition-all flex items-center justify-center gap-2.5"
            >
